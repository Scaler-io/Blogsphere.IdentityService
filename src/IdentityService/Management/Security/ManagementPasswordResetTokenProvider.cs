...
public class ManagementPasswordResetTokenProviderOptions : DataProtectionTokenProviderOptions
{
}

public class ManagementPasswordResetTokenProvider<TUser>(
    IDataProtectionProvider dataProtectionProvider,
    IOptions<ManagementPasswordResetTokenProviderOptions> options,
    ...
    where TUser : ManagementUser
{
    ...
    public override async Task<bool> ValidateAsync(string purpose, string token, UserManager<TUser> manager, TUser user)
    {
        try
        {
            ...
            _logger.Here().Information("Password reset token validation result for management user {UserId}: {IsValid}", user.Id, isValid);
            return isValid;
        }
        catch (Exception ex)
        {
            _logger.Here().Error(ex, "Error validating password reset token for management user {UserId}", user.Id);
            return false;
        }
    }
} 